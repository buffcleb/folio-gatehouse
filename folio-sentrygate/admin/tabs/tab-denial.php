...
function rbfa_render_tab_denial() {
    ...
    if ( $f_label !== '' ) {
        ...
        $screens = $wpdb->get_results( $wpdb->prepare( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name from $wpdb->prefix, not user input
            "SELECT * FROM $msg_table WHERE label LIKE %s ORDER BY label ASC LIMIT %d OFFSET %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name from $wpdb->prefix, not user input
        ...
        $screens = $wpdb->get_results( $wpdb->prepare( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name from $wpdb->prefix, not user input
            "SELECT * FROM $msg_table ORDER BY label ASC LIMIT %d OFFSET %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- table name from $wpdb->prefix, not user input
                ...
                <div style="background:#f0f6fb; border:1px solid #c3d9ee; border-radius:4px;
                    ...
                    <div style="margin-top:10px; border:1px solid #c3d9ee; border-radius:4px; background:#fff;">
                        <button type="button"
                            ...
                            <code style="background:#e3eef9; padding:2px 6px; border-radius:3px; user-select:all;">[fsg_login_link]</code>
                    ...
                    <div style="margin-top:6px; border:1px solid #c3d9ee; border-radius:4px; background:#fff;">
                        <button type="button"
                                ...
                                style="width:100%; text-align:left; background:none; border:none;
                                       padding:8px 12px; cursor:pointer; display:flex; align-items:center; gap:8px;">
                            <span class="rbfa-sc-arrow" style="font-size:10px; color:#888;">▶</span>
                            <code style="background:#e3eef9; padding:2px 6px; border-radius:3px; user-select:all;">[fsg_zone_link]</code>
                            <span style="color:#555;">&nbsp;— Login and return to the <strong>zone page</strong></span>
                        </button>
                        <div style="display:none; padding:0 12px 10px; border-top:1px solid #e3eef9;">
                            <p style="margin:8px 0 4px; color:#444;">
                                Same login / logout behaviour as <code>[fsg_login_link]</code>, but after
                    ...
                    <p>
                        <label style="font-weight:600; display:block; margin-bottom:4px;">
                            ...
                            <span style="font-weight:normal; color:#666;">
                                — used by <code>[fsg_login_link]</code>. Leave blank to use
